...
{
    public class DDDAdapter : ICloseable
    {
        ...
        public delegate void UpdateCPEDelegate(String dm, double ffCpe, double ttCpe, int timeMs);
        ...
        private UpdateCPEDelegate _cpeUpdateCallback = null;
        ...
        public void SetCPECallback(UpdateCPEDelegate cb)
        {
            _cpeUpdateCallback = cb;
        }
        ...
        public void T_Connect(object p)
        {
            ...
            lock (_dddLock)
            {
                ...
                _dddConnection.AddEventCallback("SEAMATE_UpdateCPE", new DDDServerConnection.ProcessSimulationEvent(UpdateCPE));
        ...
        protected void UpdateCPE(SimulationEvent ev)
        {
            //DM_ID, FF_CPE, TT_CPE
            String dm;
            double ffCpe, ttCpe;
            int timeMs;
            dm = ((StringValue)ev["DM_ID"]).value;
            ffCpe = ((DoubleValue)ev["FF_CPE"]).value;
            ttCpe = ((DoubleValue)ev["TT_CPE"]).value;
            timeMs = ((IntegerValue)ev["Time"]).value;

            if (_cpeUpdateCallback != null)
            {
                _cpeUpdateCallback(dm, ffCpe, ttCpe, timeMs);
            }
        }
        protected void AttackSucceeded(SimulationEvent ev)
        { //ObjectID, TargetID, NewState
            String state = ((StringValue)ev["NewState"]).value;
            String attackerID = ((StringValue)ev["ObjectID"]).value;
            if (_revealedObjects.ContainsKey(attackerID))
            {
                SeamateObject attacker = this._revealedObjects[attackerID];
                if (attacker.Owner == "Pirate DM")
                {

                    if (!attacker.firstAttackFinished)
                    {
                        attacker.firstAttackFinished = true;
                    }
                    else if (!attacker.secondAttackFinished)
                    {
                        attacker.secondAttackFinished = true;
                        if (isInEntryRegion(attacker))
                        {
                            Console.WriteLine("Pirate vessel " + attacker.ID + " finished attack in entry region, turning state to Dead.");
                            _dddConnection.SendStateChange(attacker.ID, "Dead");
                        }
                        else
                        {
                            attacker.resetAttacks();
                        }
            ...
            ScheduleAttack(attackerID, target, 0);//schedule it as immediate.
        }
        ...
        private PolygonValue WholeScreenMinusSomalia = null;
        ...
        public class SeamateObject : SimObject
        {
            ...
            public Boolean firstAttackFinished = false;
            public Boolean secondAttackFinished = false;
            ...
            internal void SetAttributes(AttributeCollectionValue atts)
            {
                ...
                if (atts.attributes.ContainsKey("Intent"))
                {
                    this.ObjectName = ((StringValue)atts.attributes["Intent"]).value;
                }
            }
        }
        /// <summary>
        ...
        public void SendStimulusEvent(String itemID, String dmID, String objectID, String stimulusType, int time, double findFixDifficulty, double trackTargetDifficulty)
        {
            ...
            ((DoubleValue)ev["FFDifficulty"]).value = findFixDifficulty;
            ((DoubleValue)ev["TTDifficulty"]).value = trackTargetDifficulty;
        ...
        public void SendItemInfo(String dmId, String itemId, double ffDiff, double ttDiff, int time)
        {
            SimulationEvent ev = SimulationEventFactory.BuildEvent(ref _simModel, "SEAMATE_ItemStart");

            ((StringValue)ev["ItemID"]).value = itemId;
            ((StringValue)ev["DM_ID"]).value = dmId;
            ((DoubleValue)ev["FFDifficulty"]).value = ffDiff;
            ((DoubleValue)ev["TTDifficulty"]).value = ttDiff;
            ((IntegerValue)ev["Time"]).value = time;

using System;
using System.Collections.Generic;
using System.ComponentModel;
using System.Drawing;
using System.Data;
using System.Text;
using System.Windows.Forms;

namespace VisualScenarioGenerator.Dialogs
{
    public partial class Ctl_Node_LaunchEvent : UserControl, ICtl_ContentPane__OutboundUpdate, ICtl_Node
    {
        public Ctl_Node_LaunchEvent()
        {
            InitializeComponent();
        }


        #region IVSG_ControlStateOutboundUpdate Members

        void ICtl_ContentPane__OutboundUpdate.Update(Control control, object object_data)
        {
            throw new Exception("The method or operation is not implemented.");
        }

        #endregion

        private void ctl_Node_EngramRange1_Load(object sender, EventArgs e)
        {

        }

        #region ICtl_Node Members

        public void SetNodeData(string id, int tick)
        {
            this.ctl_Node1.ID = id;
            this.ctl_Node1.Tick = tick;
        }

        public string GetNodeId()
        {
            return ctl_Node1.ID;
        }

        public int GetTick()
        {
            return ctl_Node1.Tick;
        }

        #endregion

    }
}

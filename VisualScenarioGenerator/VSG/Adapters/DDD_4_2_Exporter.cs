...
{
    public class DDD_4_2_Exporter : ControllerToXmlAdapter<IXPathNavigable>
    {
        private DDD_4_2_ExporterSettings settings;
        private XPathExpression expression;
        private List<string> createdObjectIds = new List<string>();

        public DDD_4_2_Exporter(DDD_4_2_ExporterSettings settings) : base((ControllerToXmlSettings)settings)
        {
            this.settings = settings;
            createdObjectIds = new List<string>();
        }

        public DDD_4_2_Exporter(Controller controller,
                                RootController rootController,
                                Boolean treeDataFormat) : base(controller, rootController, treeDataFormat)
        {
            this.settings = new DDD_4_2_ExporterSettings(controller, rootController, treeDataFormat);
            createdObjectIds = new List<string>();
        }
        ...
        public override IXPathNavigable Process()
        {
            ...
            expression = navigator.Compile("Component[@Type='Engram'] | " +
                                           ...
                                           "Component[@Type='WeaponLaunchEvent'] | " +
                                           "Component[@Type='OpenChatRoomEvent'] | " +
                                           "Component[@Type='CloseChatRoomEvent'] | " +
                                           "Component[@Type='SendChatMessageEvent'] | " +
                                           "Component[@Type='SendVoiceMessageEvent'] | " +
                                           "Component[@Type='SendVoiceMessageToUserEvent'] | " +
                                           "Component[@Type='OpenVoiceChannelEvent'] | " +
                                           "Component[@Type='CloseVoiceChannelEvent'] | " +
            ...
            if (CheckCreateEvents(navigator))
            {
        ...
        private Boolean CheckCreateEvents(XPathNavigator nav)
        {
            // for every create event pair
            // we need to ask if the species type of the first create event allows 
            // a. the species of the second create event
            // b. the # of species of the second create event doesn't go over the allowed count value for the species of the 
            // first create event

            StringBuilder outputMessages = new StringBuilder();

            foreach (XPathNavigator createEvent1 in nav.Select("/LinkTypes/Link[@type='CreateEventKind']/Component"))
            {
                String createEvent1Name = createEvent1.GetAttribute("Name", "");
                String createEvent1ID = createEvent1.GetAttribute("ID", "");
                Int32 species1ID = -1;
                String species1Type = GetSpeciesFromCreateEvent(nav, createEvent1ID, out species1ID);
                if (species1Type != null)
                {
                    // for each species type, store the # of instances found
                    Dictionary<String, Int32> foundTypeCounts = new Dictionary<String, Int32>();
                    foreach (XPathNavigator createEvent2 in nav.Select("/LinkTypes/Link[@type='CreateEventSubplatform']/Component[@ID='" + createEvent1ID + "']/Component"))
                    {
                        String createEvent2ID = createEvent2.GetAttribute("ID", "");
                        Int32 species2ID = -1;
                        String species2Type = GetSpeciesFromCreateEvent(nav, createEvent2ID, out species2ID);
                        if (species2Type != null)
                        {
                            if (!foundTypeCounts.ContainsKey(species2Type))
                            {
                                foundTypeCounts.Add(species2Type, 1);
                            }
                            else
                            {
                                foundTypeCounts[species2Type] = ++foundTypeCounts[species2Type];
                            }
                        }
                    }
                    // now check the capacity linktype to find out what we allow
                    String dynamicSubplatformCapacity = "SpeciesSubplatformCapacity_" + species1ID;

                    Dictionary<String, Int32> allowedTypeCounts = new Dictionary<String, Int32>();
                    foreach (XPathNavigator allowedSubplatform in nav.Select("/LinkTypes/Link[@type='" + dynamicSubplatformCapacity + "']/Component[@ID='" + species1ID + "']/Component"))
                    {
                        Int32 subplatformID = Int32.Parse(allowedSubplatform.GetAttribute("ID", ""));
                        if (subplatformID != species1ID) // skip ourselves, if necessary
                        {
                            String subplatformType = allowedSubplatform.GetAttribute("Name", "");
                            Int32 maxCount = Int32.Parse(allowedSubplatform.SelectSingleNode
                                ("LinkParameters/Parameter/Parameter[@displayedName='Count']/@value").Value);
                            if (maxCount > 0)
                            {
                                allowedTypeCounts.Add(subplatformType, maxCount);
                            }
                        }
                    }

                    // Compare the found counts and types versus the allowed counts and types
                    foreach (String foundType in foundTypeCounts.Keys)
                    {
                        if (!allowedTypeCounts.ContainsKey(foundType))
                        {
                            outputMessages.Append("Create event ");
                            outputMessages.Append(createEvent1Name);
                            outputMessages.Append(" owned by DM ");
                            outputMessages.Append(GetCreateEventOwner(nav, createEvent1ID));
                            outputMessages.Append(" of species ");
                            outputMessages.Append(species1Type);
                            outputMessages.Append(" does not allow species ");
                            outputMessages.Append(foundType);
                            outputMessages.AppendLine(" as a subplatform ");
                        }
                        else if (foundTypeCounts[foundType] > allowedTypeCounts[foundType])
                        {
                            outputMessages.Append("Create event ");
                            outputMessages.Append(createEvent1Name);
                            outputMessages.Append(" owned by DM ");
                            ...
                            outputMessages.Append(" of species ");
                            outputMessages.Append(species1Type);
                            outputMessages.Append(" has too many subplatforms of type ");
                            outputMessages.Append(foundType);
                            outputMessages.Append(" # allowed: ");
                            outputMessages.Append(allowedTypeCounts[foundType]);
                            outputMessages.Append(" # found: ");
                            outputMessages.AppendLine("" + foundTypeCounts[foundType]);
                        }
                    }
                }
            }
            ...
            {
                MessageBox.Show(outputMessages.ToString(), "Scenario Export Failed - Please Correct");
                return false;
            }
            else
            {
        ...
        private String GetSpeciesFromCreateEvent(XPathNavigator nav, String createEventID, out Int32 speciesID)
        {
            // Species kind (if it has one) will be the child Species here, otherwise it will be null
            XPathNavigator speciesNav = nav.SelectSingleNode("/LinkTypes/Link[@type='CreateEventKind']/Component[@ID='" + createEventID + "']/Component");
            if (speciesNav != null)
            {
                speciesID = Int32.Parse(speciesNav.GetAttribute("ID", ""));
                return speciesNav.GetAttribute("Name", "");
            }
            else
            {
                speciesID = -1;
        ...
        private String GetCreateEventOwner(XPathNavigator nav, String createEventID)
        {
            XPathNavigator dmNav = nav.SelectSingleNode("/LinkTypes/Link[@type='Scenario']/Component/Component[@ID='" + createEventID + "']/Component[@Type='DecisionMaker']");
            if (dmNav != null)
            {
                return dmNav.GetAttribute("Name", "");
            }
            else
            {
                return null;
        ...
        private void createScenario(XPathNavigator navigator, XmlDocument document)
        {
            ...
            schema.Value = @"DDDSchema_4_2.xsd";
            ...
            // ClientSideRangeRingVisibility
            XPathNavigator navRangeRingVisibility = navigator.SelectSingleNode("/LinkTypes/Link[@type='Scenario']/Component[@Type='Scenario']");
            if (navRangeRingVisibility != null)
            {
                createClientSideRangeRingVisibility(navRangeRingVisibility, document, root, navigator);
            }
            ...
            //Classifications
            XPathNodeIterator itClassifications = navigator.Select("/LinkTypes/Link[@type='Scenario']/Component[@Type='Scenario']/Component[@Type='Classification']");
            if (itClassifications.Count > 0)
            {
                // Classifications
                XmlElement classifications = document.CreateElement("Classifications");
                classifications.IsEmpty = false;

                while (itClassifications.MoveNext())
                {
                    createClassification(itClassifications.Current, document, classifications, navigator);
                }
            ...
            if (navLocal != null)
            {
                ...
                if (navAllowAssetTransfers != null)
                {
                    ...
                    if (!value.Equals(String.Empty))
                    {
                        ...
                        {
                            clientSideAssetTransfer.InnerXml = "1";
                        }
                        else
                        {
                            clientSideAssetTransfer.InnerXml = "0";
                        }
        ...
        private void createClientSideRangeRingVisibility(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            // Create Description
            if (navLocal != null)
            {
                XPathNavigator navRangeRingVisibility = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Range Ring Display Type' and @category='Scenario']");
                if (navRangeRingVisibility != null)
                {
                    XmlElement rangeRingDisplay = document.CreateElement("ClientSideRangeRingVisibility");
                    rangeRingDisplay.IsEmpty = false;
                    String value = navRangeRingVisibility.GetAttribute("value", navRangeRingVisibility.NamespaceURI);
                    if (value.Equals(String.Empty))
                    {
                        value = "Full";
                    }
                    rangeRingDisplay.InnerXml = value;
                    parentElement.AppendChild(rangeRingDisplay);
                }
            }
        }
        ...
        private void createPlayfield(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            ...
            // ClientSideDisplayLabels
            XPathNavigator navDisplayLabels = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Display Labels' and @category='Playfield']");
            if (navDisplayLabels != null)
            {
                XmlElement labels = document.CreateElement("ClientSideDisplayLabels");
                labels.IsEmpty = false;
                String value = navDisplayLabels.GetAttribute("value", navLocal.NamespaceURI);
                if (!value.Equals(String.Empty))
                {
                    labels.InnerXml = value;
                    playField.AppendChild(labels);
                }
            }

            // ClientSideTagDisplay
            XPathNavigator navDisplayTags = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Display Tags' and @category='Playfield']");
            if (navDisplayTags != null)
            {
                XmlElement tags = document.CreateElement("ClientSideTagDisplay");
                tags.IsEmpty = false;
                String value = navDisplayTags.GetAttribute("value", navLocal.NamespaceURI);
                if (!value.Equals(String.Empty))
                {
                    tags.InnerXml = value;
                    playField.AppendChild(tags);
                }
            }
        }
        ...
        {
            bool hasReferencePoint = (navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='ReferencePoint' and @category='Location']") != null);
            // Create ActiveRegion
            ...
            // Vertex
            XPathNavigator navVertices = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Polygon Points' and @category='Location']");
            if (navVertices == null || hasReferencePoint)
            {
                navVertices = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Relative Polygon Points' and @category='Location']");
            }
            if (navVertices != null)
            {
                String polygonPoints = navVertices.GetAttribute("value", navLocal.NamespaceURI);
            ...
            // Reference Point
            XPathNavigator navRefPt = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='ReferencePoint' and @category='Location']");
            if (navRefPt != null)
            {
                XmlElement pt = document.CreateElement("ReferencePoint");
                pt.IsEmpty = false;
                String value = navRefPt.GetAttribute("value", navRefPt.NamespaceURI);
                if (!value.Equals(String.Empty))
                {
                    pt.InnerXml = value.Replace(","," ");
                    activeRegion.AppendChild(pt);
                }
            }

            // IsDynamic
            XPathNavigator navDynamic = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Is this a Dynamic Region' and @category='Active Region']");
            if (navDynamic != null)
            {
                XmlElement isDynamic = document.CreateElement("IsDynamicRegion");
                isDynamic.IsEmpty = false;
                String value = navDynamic.GetAttribute("value", navDynamic.NamespaceURI);
                if (!value.Equals(String.Empty))
                {
                    isDynamic.InnerXml = value;
                    activeRegion.AppendChild(isDynamic);
                }
            }
            ...
            if (navColor != null)
            {
                ...
                c = (System.Drawing.Color)cc.ConvertFromString(value);
                if (!value.Equals(String.Empty))
                {
                    color.InnerXml = c.Name;
        ...
        private void createClassification(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            // Create Classification
            XmlElement classification = document.CreateElement("Classification");
            classification.IsEmpty = false;

            parentElement.AppendChild(classification);
        ...
        private void createDecisionMaker(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            ...
            //ReportsTo
            String reportsToDecisionMakerId = navLocal.GetAttribute("ID", navLocal.NamespaceURI);
            //XPathNodeIterator itReportsToDecisionMaker = navGlobal.Select(String.Format("/LinkTypes/Link[@type='DecisionMakerReportsTo']//Component[@ID='{0}' and @Type='DecisionMaker']", reportsToDecisionMakerId));
            //XPathNavigator navReportsToDecisionMaker = navGlobal;
            //while(itReportsToDecisionMaker.MoveNext()) {
            XPathNavigator navReportsToDecisionMaker = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@type='DecisionMakerReportsTo']//Component[@ID='{0}' and @Type='DecisionMaker']", reportsToDecisionMakerId));

            if (navReportsToDecisionMaker != null)
            {

                XmlElement reportsTo = document.CreateElement("ReportsTo");
                reportsTo.IsEmpty = false;
                XPathNodeIterator itReportsTo = navReportsToDecisionMaker.Select("Component[@Type='DecisionMaker']");
                List<String> reportsToList = new List<String>();
                while (itReportsTo.MoveNext())
                {
                    String value = itReportsTo.Current.GetAttribute("Name", itReportsTo.Current.NamespaceURI);
                    if (!value.Equals(String.Empty))
                    {
                        reportsToList.Add(value);
                    }
                }
                if (reportsToList.Count > 0)
                {
                    reportsTo.InnerXml = String.Join(",", reportsToList.ToArray());
                    decisionMaker.AppendChild(reportsTo);
                }
            }


            // CanTransfer
            XPathNavigator navCanTransfer = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='CanTransferAssets' and @category='DecisionMaker']");
            if (navCanTransfer != null)
            {
                XmlElement canTransfer = document.CreateElement("CanTransfer");
                canTransfer.IsEmpty = false;
                String value = navCanTransfer.GetAttribute("value", navCanTransfer.NamespaceURI);
                if (!value.Equals(String.Empty))
                {
                    canTransfer.InnerXml = value;
                    decisionMaker.AppendChild(canTransfer);
                }
            }

            // CanForceTransfers
            XPathNavigator navCanForceTransfers = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='CanForceAssetTransfers' and @category='DecisionMaker']");
            if (navCanForceTransfers != null)
            {
                XmlElement canForceTransfers = document.CreateElement("CanForceTransfers");
                canForceTransfers.IsEmpty = false;
                String value = navCanForceTransfers.GetAttribute("value", navCanForceTransfers.NamespaceURI);
                if (!value.Equals(String.Empty))
                {
                    canForceTransfers.InnerXml = value;
                    decisionMaker.AppendChild(canForceTransfers);
                }
            }
            ...
            //CanChat
            String canChatDecisionMakerId = navLocal.GetAttribute("ID", navLocal.NamespaceURI);
            //XPathNodeIterator itCanChatDecisionMaker = navGlobal.Select(String.Format("/LinkTypes/Link[@type='DecisionMakerCanChat']//Component[@ID='{0}' and @Type='DecisionMaker']", canChatDecisionMakerId));
            //XPathNavigator navCanChatDecisionMaker = navGlobal;

            //while (itCanChatDecisionMaker.MoveNext())
            //{
            XPathNavigator navCanChatDecisionMaker = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='DecisionMakerCanChat']/Component[@ID='{0}' and @Type='DecisionMaker']", canChatDecisionMakerId));
            
            if (navCanChatDecisionMaker != null)
            {

                XmlElement canChat = document.CreateElement("CanChat");
                canChat.IsEmpty = false;
                XPathNodeIterator itCanChat = navCanChatDecisionMaker.Select("Component[@Type='DecisionMaker']");
                List<String> canChatToList = new List<String>();
                while (itCanChat.MoveNext())
                {
                    String value = itCanChat.Current.GetAttribute("Name", itCanChat.Current.NamespaceURI);
                    if (!value.Equals(String.Empty))
                    {
                        canChatToList.Add(value);
                    }
                }
                if (canChatToList.Count > 0)
                {
                    canChat.InnerXml = String.Join(",", canChatToList.ToArray());
                    decisionMaker.AppendChild(canChat);
                }
            }

            //CanWhiteboard
            String canWhiteboardDecisionMakerId = navLocal.GetAttribute("ID", navLocal.NamespaceURI);
            XPathNavigator navCanWhiteboardDecisionMaker = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='DecisionMakerCanWhiteboard']/Component[@ID='{0}' and @Type='DecisionMaker']", canWhiteboardDecisionMakerId));

            if (navCanWhiteboardDecisionMaker != null)
            {

                XmlElement canWhiteboard = document.CreateElement("CanWhiteboard");
                canWhiteboard.IsEmpty = false;
                XPathNodeIterator itCanWhiteboard = navCanWhiteboardDecisionMaker.Select("Component[@Type='DecisionMaker']");
                List<String> canWhiteboardToList = new List<String>();
                while (itCanWhiteboard.MoveNext())
                {
                    String value = itCanWhiteboard.Current.GetAttribute("Name", itCanWhiteboard.Current.NamespaceURI);
                    if (!value.Equals(String.Empty))
                    {
                        canWhiteboardToList.Add(value);
                    }
                }
                if (canWhiteboardToList.Count > 0)
                {
                    canWhiteboard.InnerXml = String.Join(",", canWhiteboardToList.ToArray());
                    decisionMaker.AppendChild(canWhiteboard);
                }
            }

            //CanSpeak
            String canSpeakDecisionMakerId = navLocal.GetAttribute("ID", navLocal.NamespaceURI);
            XPathNavigator navCanSpeakDecisionMaker = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='DecisionMakerCanSpeak']/Component[@ID='{0}' and @Type='DecisionMaker']", canSpeakDecisionMakerId));

            if (navCanSpeakDecisionMaker != null)
            {

                XmlElement canSpeak = document.CreateElement("CanSpeak");
                canSpeak.IsEmpty = false;
                XPathNodeIterator itCanSpeak = navCanSpeakDecisionMaker.Select("Component[@Type='DecisionMaker']");
                List<String> canSpeakToList = new List<String>();
                while (itCanSpeak.MoveNext())
                {
                    String value = itCanSpeak.Current.GetAttribute("Name", itCanSpeak.Current.NamespaceURI);
                    if (!value.Equals(String.Empty))
                    {
                        canSpeakToList.Add(value);
                    }
                }
                if (canSpeakToList.Count > 0)
                {
                    canSpeak.InnerXml = String.Join(",", canSpeakToList.ToArray());
                    decisionMaker.AppendChild(canSpeak);
                }
            }

            // IsObserver
            XPathNavigator navIsObserver = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='IsObserver' and @category='DecisionMaker']");
            if (navIsObserver != null)
            {
                XmlElement isObserver = document.CreateElement("IsObserver");
                isObserver.IsEmpty = false;
                String value = navIsObserver.GetAttribute("value", navIsObserver.NamespaceURI);
                if (!value.Equals(String.Empty))
                {
                    isObserver.InnerXml = value;
                    decisionMaker.AppendChild(isObserver);
                }
            }
        ...
        private void createSpecies(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            ...
            // Size
            XPathNavigator navSize = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='CollisionRadius' and @category='Species']");
            if (navSize != null)
            {
            ...
            XPathNavigator navDefaultClassification = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='DefaultClassification' and @category='Species']");
            if (navDefaultClassification != null)
            {
                XmlElement defClassification = document.CreateElement("DefaultClassification");
                defClassification.IsEmpty = false;
                String value = navDefaultClassification.GetAttribute("value", navDefaultClassification.NamespaceURI);
                if (!value.Equals(String.Empty))
                {
                    defClassification.InnerXml = value;
                    species.AppendChild(defClassification);
                }
            }

            //TODO: ClassificationDisplayRules
            XPathNavigator navClassificationRules = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='ClassificationDisplayRules' and @category='Species']");
            if (navClassificationRules != null)
            {
                String value = navClassificationRules.GetAttribute("value", navClassificationRules.NamespaceURI);
                
                XmlElement defClassificationRules = document.CreateElement("ClassificationDisplayRules");
                defClassificationRules.IsEmpty = false;
                
                if (!value.Equals(String.Empty))
                {
                    ClassificationDisplayRules rules = new ClassificationDisplayRules();
                    rules.Rules = ClassificationDisplayRules.FromXML(value);
                    StringBuilder ruleString = new StringBuilder();
                    foreach (ClassificationDisplayRule r in rules.Rules)
                    {
                        ruleString.Append(r.ToXML());
                    }
                    defClassificationRules.InnerXml = ruleString.ToString();
                    species.AppendChild(defClassificationRules);
                }
            }

            //CanOwn
            String canOwnId = navLocal.GetAttribute("ID", navLocal.NamespaceURI);
            XPathNodeIterator itCanOwn = navGlobal.Select(String.Format("/LinkTypes/Link[@type='SpeciesDMCanOwn']//Component[@ID='{0}' and @Type='Species']", canOwnId));
            //XPathNavigator navReportsToDecisionMaker = navGlobal;
            while (itCanOwn.MoveNext())
            {

                XmlElement canOwn = document.CreateElement("CanOwn");
                canOwn.IsEmpty = false;
                XPathNodeIterator it = itCanOwn.Current.Select("Component[@Type='DecisionMaker']");
                List<String> canOwnList = new List<String>();
                while (it.MoveNext())
                {
                    String value = it.Current.GetAttribute("Name", it.Current.NamespaceURI);
                    if (!value.Equals(String.Empty))
                    {
                        canOwnList.Add(value);
                    }
                }
                if (canOwnList.Count > 0)
                {
                    canOwn.InnerXml = String.Join(",", canOwnList.ToArray());
                    species.AppendChild(canOwn);
                }
            }

            // LaunchedByOwner
            XPathNavigator navLaunchedByOwner = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='LaunchedByOwner' and @category='Species']");
            if (navLaunchedByOwner != null)
            {
                XmlElement launchedByOwner = document.CreateElement("LaunchedByOwner");
                launchedByOwner.IsEmpty = false;
                String value = navLaunchedByOwner.GetAttribute("value", navIsWeapon.NamespaceURI);
                if (!value.Equals(String.Empty))
                {
                    launchedByOwner.InnerXml = value;
                    species.AppendChild(launchedByOwner);
                }
            }

            // SubplatformCapacity
            String subplatCapID = navLocal.GetAttribute("ID", navLocal.NamespaceURI);
            String dynamicLink = String.Format("SpeciesSubplatformCapacity_{0}", subplatCapID);
            XPathNodeIterator itSubplatformCapacity = navGlobal.Select(String.Format("/LinkTypes/Link[@type='{0}']/Component[@ID='{1}' and @Type='Species']/Component[@Type='Species']", dynamicLink, subplatCapID));
            while (itSubplatformCapacity.MoveNext())
            {

                XmlElement subpCap = document.CreateElement("SubplatformCapacity");
                
                XPathNavigator navSubpClass = itSubplatformCapacity.Current.SelectSingleNode("Component[@Type='Species']");
                String subpClass = itSubplatformCapacity.Current.GetAttribute("Name", navLaunchedByOwner.NamespaceURI);
                XPathNavigator navSubpCount = itSubplatformCapacity.Current.SelectSingleNode("LinkParameters/Parameter/Parameter");
                String count = navSubpCount.GetAttribute("value", navLaunchedByOwner.NamespaceURI);


                XmlElement subpClassElement = document.CreateElement("SpeciesName");
                subpClassElement.InnerText = subpClass;
                XmlElement subpCountElement = document.CreateElement("Count");
                subpCountElement.InnerText = count;

                if (count == "0")
                {
                    continue; //we don't want to output all species' that have no subplatforms dockable.
                }

                subpCap.AppendChild(subpClassElement);
                subpCap.AppendChild(subpCountElement);
                species.AppendChild(subpCap);
                //canOwn.IsEmpty = false;
                //XPathNodeIterator it = itCanOwn.Current.Select("Component[@Type='DecisionMaker']");
                //List<String> canOwnList = new List<String>();
                //while (it.MoveNext())
        ...
        private void createStateParameters(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            ...
            // EngagementDuration
            XPathNavigator navOverrideEngagementDuration = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='OverrideEngagementDuration' and @category='State']");
            if (navOverrideEngagementDuration != null)
            {
                if (Boolean.Parse(navOverrideEngagementDuration.GetAttribute("value", navOverrideEngagementDuration.NamespaceURI)) | (!inheritedSpecies & stateName.Equals("FullyFunctional")))
                {
                    XPathNavigator navEngagementDuration = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='EngagementDuration' and @category='State']");
                    if (navEngagementDuration != null)
                    {
                        XmlElement engagementDuration = document.CreateElement("EngagementDuration");
                        engagementDuration.IsEmpty = false;
                        String value = navEngagementDuration.GetAttribute("value", navEngagementDuration.NamespaceURI);
                        if (!value.Equals(String.Empty))
                        {
                            if (value != VSG.Program._defaultEngagementDuration.ToString())
                            {
                                engagementDuration.InnerXml = value;
                                stateParameters.AppendChild(engagementDuration);
                            }
                        }
                    }
                }
            ...
            if (navLocal != null)
            {
                switch (navLocal.GetAttribute("Type", navLocal.NamespaceURI))
                {
                    ...
                    case "CloseChatRoomEvent":
                        ...
                        break;
                    case "SendChatMessageEvent":
                        createSendChatMessage(navLocal, document, parentElement, navGlobal);
                        break;
                    case "SendVoiceMessageEvent":
                        createSendVoiceMessage(navLocal, document, parentElement, navGlobal);
                        break;
                    case "SendVoiceMessageToUserEvent":
                        createSendVoiceMessageToUser(navLocal, document, parentElement, navGlobal);
                        break;
                    case "CloseVoiceChannelEvent":
                        createCloseVoiceChannel(navLocal, document, parentElement, navGlobal);
                        break;
                    case "CompletionEvent":
                        createCompletion_Event(navLocal, document, parentElement, navGlobal);
                        break;
                    case "CreateEvent":
                        createCreate_Event(navLocal, document, parentElement, navGlobal, null);
                        break;
                    case "FlushEvent":
                    ...
                    case "WeaponLaunchEvent":
                        createWeaponLaunch_Event(navLocal, document, parentElement, navGlobal);
                        break;
                    case "MoveEvent":
                    ...
                    case "OpenVoiceChannelEvent":
                        createOpenVoiceChannel(navLocal, document, parentElement, navGlobal);
                        break;
                    case "ReiterateEvent":
        ...
        private void createCreate_Event(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal, String[] parentIDs)
        {
            String thisObjectId = navLocal.GetAttribute("Name", navLocal.NamespaceURI);
            if (createdObjectIds.Contains(thisObjectId))
            {
                return; //this object has already been created.
            }
            
            String createEventId = navLocal.GetAttribute("ID", navLocal.NamespaceURI);

            //if no parents defined, but you can prove there are some, return and let the parent define this unit
            if (parentIDs == null)
            {
                if (navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@type='CreateEventSubplatform']/Component[@Type='CreateEvent']/Component[@ID='{0}' and @Type='CreateEvent']", createEventId)) != null)
                {
                    return; //let the parent handle creation.
                }
            }

            //navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='225' and @type='CreateEventSubplatform']/Component[@ID='225']/Component[@ID='211' and @Type='CreateEvent']", createEventId)).InnerXml
            ...
            //This section checks that all of this event's subplatform's have already been Created.  This is recursive now.
            XPathNavigator recursiveSubplatformMembers = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='CreateEventSubplatform']/Component[@ID='{0}' and @Type='CreateEvent']", createEventId));

            //if this object is a subplatform, adapt the XPath accordingly.
            StringBuilder newXPath = new StringBuilder();
            if (parentIDs != null)
            {
                List<string> previousParents = new List<string>(parentIDs);
                if (previousParents.Count > 0)
                {
                    newXPath = new StringBuilder(String.Format("/LinkTypes/Link[@id='{0}' and @type='CreateEventSubplatform']/Component[@ID='{0}' and @Type='CreateEvent']", previousParents[0]));
                    if (previousParents.Count > 1)
                    {
                        for (int x = 1; x < previousParents.Count; x++)
                        {
                            newXPath.AppendFormat("/Component[@ID='{0}' and @Type='CreateEvent']", previousParents[x]);
                        }
                    }
                    newXPath.AppendFormat("/Component[@ID='{0}' and @Type='CreateEvent']", createEventId);
                    recursiveSubplatformMembers = navGlobal.SelectSingleNode(newXPath.ToString());
                }
            }

            if (recursiveSubplatformMembers != null)
            {
                XPathNodeIterator itMembers = recursiveSubplatformMembers.Select("Component[@Type='CreateEvent']");

                while (itMembers.MoveNext())
                {
                    String value = itMembers.Current.GetAttribute("Name", itMembers.Current.NamespaceURI);

                    if (!createdObjectIds.Contains(value))
                    {
                        //recursively call Create Event
                        List<string> parents = new List<string>();
                        if (parentIDs != null)
                        {
                            parents.AddRange(parentIDs);
                        }
                        parents.Add(createEventId);
                        createCreate_Event(itMembers.Current, document, parentElement, navGlobal, parents.ToArray());
            ...
            //String networkId = navLocal.GetAttribute("ID", navLocal.NamespaceURI);
            XPathNavigator subplatformMembers = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='CreateEventSubplatform']/Component[@ID='{0}' and @Type='CreateEvent']", createEventId));
            if (newXPath.Length > 4) //4 is arbitrary, just > 0 is fine.
            {
                subplatformMembers = navGlobal.SelectSingleNode(newXPath.ToString());
            }
            if (subplatformMembers != null)
            {
                XmlElement member = document.CreateElement("Subplatform");
                member.IsEmpty = false;
                XPathNodeIterator itMembers = subplatformMembers.Select("Component[@Type='CreateEvent']");
                List<String> memberList = new List<String>();
                while (itMembers.MoveNext())
                {
                    String value = itMembers.Current.GetAttribute("Name", itMembers.Current.NamespaceURI);
                ...
                {
                    member.InnerXml = String.Join(",", memberList.ToArray());
                    create_Event.AppendChild(member);
                }
            }

            createdObjectIds.Add(thisObjectId);
        }
        ...
        private void createLaunch_Event(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            ...
            {
                XPathNavigator navSubplatform = navLaunchEventSubplatform.SelectSingleNode("Component[@Type='CreateEvent']");
                if (navSubplatform != null)
                {
                    XmlElement child = document.CreateElement("Child");
                    child.IsEmpty = false;
                    String value = navSubplatform.GetAttribute("Name", navSubplatform.NamespaceURI);
                    if (!value.Equals(String.Empty))
                    {
                        child.InnerXml = value;
                        launch_Event.AppendChild(child);
                    }
            ...
            XPathNodeIterator itStartupParameters = navLocal.Select("ComponentParameters/Parameter/Parameter[@displayedName='OverrideStealable' and @category='StartupParameters'] | " +
                                ...
                                "ComponentParameters/Parameter/Parameter[@displayedName='OverrideEngagementDuration' and @category='StartupParameters'] | " +
                ...
                while (itStartupParameters.MoveNext())
                {
                    switch (itStartupParameters.Current.GetAttribute("displayedName", itStartupParameters.Current.NamespaceURI))
                    {
                        ...
                        case "OverrideTimeToAttack":
                            ...
                            break;
                            //engagement duration
                        case "OverrideEngagementDuration":
                            if (Boolean.Parse(itStartupParameters.Current.GetAttribute("value", itStartupParameters.Current.NamespaceURI)))
                            {
                                XPathNavigator navEngagementDuration = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='EngagementDuration' and @category='StartupParameters']");
                                if (navEngagementDuration != null)
                                {
                                    XmlElement parameter = document.CreateElement("Parameter");
                                    parameter.IsEmpty = false;
                                    parameter.InnerXml = "EngagementDuration";
                                    startupParameters.AppendChild(parameter);
                                    XmlElement setting = document.CreateElement("Setting");
                                    setting.IsEmpty = false;
                                    String value = navEngagementDuration.GetAttribute("value", String.Empty);
                                    if (!value.Equals(String.Empty))
                                    {
                                        setting.InnerXml = value;
                                        startupParameters.AppendChild(setting);
        ...
        private void createWeaponLaunch_Event(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            // Create Launch_Event
            XmlElement launch_Event = document.CreateElement("WeaponLaunch_Event");
            launch_Event.IsEmpty = false;
            ...
            if (navIsUnit != null)
            {
                ...
                {
                    XPathNavigator navEventID = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='EventID']/Component[@ID='{0}' and @Type='WeaponLaunchEvent']", launchEventId));
                    if (navEventID != null)
                    {
                        XPathNavigator navUnit = navEventID.SelectSingleNode("Component[@Type='CreateEvent']");
                        if (navUnit != null)
            ...
            // EngramRange
            XPathNavigator navEventEngram = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='EventEngram']/Component[@ID='{0}' and @Type='WeaponLaunchEvent']", launchEventId));
            if (navEventEngram != null)
            {
                XPathNavigator navEngram = navEventEngram.SelectSingleNode("Component[@Type='Engram']");
                if (navEngram != null)
            ...
            // Time
            XPathNavigator navTime = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Time' and @category='WeaponLaunchEvent']");
            if (navTime != null)
            {
                XmlElement time = document.CreateElement("Time");
                time.IsEmpty = false;
            ...
            // Child
            XPathNavigator navLaunchEventSubplatform = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='WeaponLaunchEventSubplatform']/Component[@ID='{0}' and @Type='WeaponLaunchEvent']", launchEventId));
            if (navLaunchEventSubplatform != null)
            {
                XPathNavigator navSubplatform = navLaunchEventSubplatform.SelectSingleNode("Component[@Type='CreateEvent']");
                if (navSubplatform != null)
            ...
            // Target
            XPathNavigator navLaunchEventTarget = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='WeaponLaunchEventTarget']/Component[@ID='{0}' and @Type='WeaponLaunchEvent']", launchEventId));
            if (navLaunchEventTarget != null)
            {
                XPathNavigator navTarget = navLaunchEventTarget.SelectSingleNode("Component[@Type='CreateEvent']");
                if (navTarget != null)
                {
                    XmlElement target = document.CreateElement("Target");
                    target.IsEmpty = false;
                    String value = navTarget.GetAttribute("Name", navTarget.NamespaceURI);
                    if (!value.Equals(String.Empty))
                    {
                        target.InnerXml = value;
                        launch_Event.AppendChild(target);
                    }
                }
            }
            ...
            // InitialState
            XPathNavigator navInitialState = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='InitialState' and @category='WeaponLaunchEvent']");
            if (navInitialState != null)
            {
                XmlElement initialState = document.CreateElement("InitialState");
                initialState.IsEmpty = false;
        ...
        private void createReveal_Event(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            ...
            // InitialTag
            XPathNavigator navInitialTag = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='InitialTag' and @category='StartupParameters']");
            if (navInitialTag != null)
            {
                XmlElement initialTag = document.CreateElement("InitialTag");
                initialTag.IsEmpty = false;
                String value = navInitialTag.GetAttribute("value", navInitialTag.NamespaceURI);

                if (!value.Equals(String.Empty))
                {
                    initialTag.InnerXml = value;
                    reveal_Event.AppendChild(initialTag);
                }

                
            }

            XPathNavigator navLinkedDynamicRegion = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='LinkObjectToRegion']/Component/Component[@Type='ActiveRegion']", revealEventId));
            ...
            XPathNodeIterator itStartupParameters = navLocal.Select("ComponentParameters/Parameter/Parameter[@displayedName='OverrideStealable' and @category='StartupParameters'] | " +
                                ...
                                "ComponentParameters/Parameter/Parameter[@displayedName='OverrideFuelConsumption' and @category='StartupParameters'] | " +
                                "ComponentParameters/Parameter/Parameter[@displayedName='OverrideIcon' and @category='StartupParameters'] | " +
                                "ComponentParameters/Parameter/Parameter[@displayedName='OverrideObjectName' and @category='StartupParameters'] | " +
                                "ComponentParameters/Parameter/Parameter[@displayedName='OverrideFuelDepletionState' and @category='StartupParameters']");
            if (itStartupParameters.Count > 0 || navLinkedDynamicRegion != null)
            {
                XmlElement startupParameters = document.CreateElement("StartupParameters");
                startupParameters.IsEmpty = false;
                ...
                //LinkedRegion
                if (navLinkedDynamicRegion != null)
                {
                    String regionID = navLinkedDynamicRegion.GetAttribute("Name", navLinkedDynamicRegion.NamespaceURI);
                    if (regionID != string.Empty)
                    {
                        XmlElement parameter = document.CreateElement("Parameter");
                        parameter.IsEmpty = false;
                        parameter.InnerXml = "LinkedRegion";
                        startupParameters.AppendChild(parameter);
                        XmlElement setting = document.CreateElement("Setting");
                        setting.IsEmpty = false;
                        setting.InnerXml = regionID;
                        startupParameters.AppendChild(setting);                        
                    }
                }
                ...
                while (itStartupParameters.MoveNext())
                {
                    switch (itStartupParameters.Current.GetAttribute("displayedName", itStartupParameters.Current.NamespaceURI))
                    {
                        ...
                        case "OverrideIcon":
                            ...
                            break;
                        // ObjectName
                        case "OverrideObjectName":
                            if (Boolean.Parse(itStartupParameters.Current.GetAttribute("value", itStartupParameters.Current.NamespaceURI)))
                            {
                                XPathNavigator navName = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='ObjectName' and @category='StartupParameters']");
                                if (navName != null)
                                {
                                    XmlElement parameter = document.CreateElement("Parameter");
                                    parameter.IsEmpty = false;
                                    parameter.InnerXml = "ObjectName";
                                    startupParameters.AppendChild(parameter);
                                    XmlElement setting = document.CreateElement("Setting");
                                    setting.IsEmpty = false;
                                    String value = navName.GetAttribute("value", String.Empty);
                                    if (!value.Equals(String.Empty))
                                    {
                                        setting.InnerXml = value;
                                        startupParameters.AppendChild(setting);
        ...
        private void createOpenVoiceChannel(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            // Create OpenVoiceChannel
            XmlElement openVoiceChannel = document.CreateElement("OpenVoiceChannel");
            openVoiceChannel.IsEmpty = false;

            parentElement.AppendChild(openVoiceChannel);

            // Name
            XPathNavigator navName = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Name' and @category='OpenVoiceChannel']");
            if (navName != null)
            {
                XmlElement name = document.CreateElement("Name");
                name.IsEmpty = false;
                ...
                {
                    name.InnerXml = value;
                    openVoiceChannel.AppendChild(name);
                }
            }

            // Time
            XPathNavigator navTime = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Time' and @category='OpenVoiceChannel']");
            if (navTime != null)
            {
                XmlElement time = document.CreateElement("Time");
                time.IsEmpty = false;
                ...
                {
                    time.InnerXml = value;
                    openVoiceChannel.AppendChild(time);
                }
            }

            // Members
            String openVoiceChannelEventId = navLocal.GetAttribute("ID", navLocal.NamespaceURI);
            XPathNavigator navOpenVoiceChannelEventAccess = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='OpenVoiceChannelEventAccess']/Component[@ID='{0}' and @Type='OpenVoiceChannelEvent']", openVoiceChannelEventId));
            if (navOpenVoiceChannelEventAccess != null)
            {
                XmlElement access = document.CreateElement("Access");
                access.IsEmpty = false;
                XPathNodeIterator itAccess = navOpenVoiceChannelEventAccess.Select("Component[@Type='DecisionMaker']");
                List<String> accessList = new List<String>();
                while (itAccess.MoveNext())
                {
                    String value = itAccess.Current.GetAttribute("Name", itAccess.Current.NamespaceURI);
                    if (!value.Equals(String.Empty))
                    {
                ...
                {
                    access.InnerXml = String.Join(",", accessList.ToArray());
                    openVoiceChannel.AppendChild(access);
                }
            }
        }

        private void createCloseVoiceChannel(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            // Create CloseChatRoom
            XmlElement closeVoiceChannel = document.CreateElement("CloseVoiceChannel");
            closeVoiceChannel.IsEmpty = false;

            parentElement.AppendChild(closeVoiceChannel);

            // Channel
            String closeVoiceChannelEventId = navLocal.GetAttribute("ID", navLocal.NamespaceURI);
            XPathNavigator navCloseVoiceChannel = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='CloseVoiceChannel']/Component[@ID='{0}' and @Type='CloseVoiceChannelEvent']", closeVoiceChannelEventId));
            if (navCloseVoiceChannel != null)
            {
                XPathNavigator navOpenVoiceChannel = navCloseVoiceChannel.SelectSingleNode("Component[@Type='OpenVoiceChannelEvent']");
                if (navOpenVoiceChannel != null)
                {
                    XPathNavigator navOpenVoiceChannelName = navOpenVoiceChannel.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Name' and @category='OpenVoiceChannel']");
                    if (navOpenVoiceChannelName != null)
                    {
                        XmlElement name = document.CreateElement("Name");
                        name.IsEmpty = false;
                        String value = navOpenVoiceChannelName.GetAttribute("value", navOpenVoiceChannelName.NamespaceURI);
                        if (!value.Equals(String.Empty))
                        {
                            name.InnerXml = value;
                            closeVoiceChannel.AppendChild(name);
                        }
                    }
                }
            }
            ...
            // Name
            XPathNavigator navName = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Name' and @category='CloseVoiceChannel']");
            if (navName != null)
            {
                XmlElement name = document.CreateElement("Name");
                name.IsEmpty = false;
            ...
            // Time
            XPathNavigator navTime = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Time' and @category='CloseVoiceChannel']");
            if (navTime != null)
            {
                XmlElement time = document.CreateElement("Time");
                time.IsEmpty = false;
                ...
                {
                    time.InnerXml = value;
                    closeVoiceChannel.AppendChild(time);
                }
            }
        }
        ...
        private void createSendChatMessage(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            // Create SendChatMessage
            XmlElement sendChatMessage = document.CreateElement("SendChatMessage");
            sendChatMessage.IsEmpty = false;

            parentElement.AppendChild(sendChatMessage);

            // Room
            String sendChatMessageEventId = navLocal.GetAttribute("ID", navLocal.NamespaceURI);
            XPathNavigator navSendChatMessage = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='SendChatMessageRoom']/Component[@ID='{0}' and @Type='SendChatMessageEvent']", sendChatMessageEventId));
            if (navSendChatMessage != null)
            {
                XPathNavigator navOpenChatRoom = navSendChatMessage.SelectSingleNode("Component[@Type='OpenChatRoomEvent']");
                if (navOpenChatRoom != null)
                {
                    XPathNavigator navOpenChatRoomName = navOpenChatRoom.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Name' and @category='OpenChatRoom']");
                    if (navOpenChatRoomName != null)
                    {
                        ...
                        {
                            room.InnerXml = value;
                            sendChatMessage.AppendChild(room);
                        }
                    }
                }
            }
            ...
            //XPathNavigator navSendChatMessageSender = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@type='SendChatMessageSenderDM']/Component[@ID='{0}' and @Type='DecisionMaker']", sendChatMessageEventId));
            //if (navSendChatMessageSender != null)
            //{
            //    XPathNavigator navDecisionMaker = navSendChatMessageSender.SelectSingleNode("Component[@Type='DecisionMaker']");
            //    if (navDecisionMaker != null)
            //    {
            //        XmlElement from = document.CreateElement("From");
            //        from.IsEmpty = false;
            ...
            XPathNavigator navSendChatMessageSender = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='SendChatMessageSenderDM']/Component[@ID='{0}' and @Type='SendChatMessageEvent']", sendChatMessageEventId));
            //XPathNavigator navSendChatMessageSender = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@type='SendChatMessageSenderDM']/Component[@ID='{0}' and @Type='DecisionMaker']", sendChatMessageEventId));
            if (navSendChatMessageSender != null)
            {
                XPathNavigator navSender = navSendChatMessageSender.SelectSingleNode("Component[@Type='DecisionMaker']");
                if (navSender != null)
                {

                    XmlElement sender = document.CreateElement("Sender");
                    sender.IsEmpty = false;
                    String value = navSender.GetAttribute("Name", navSender.NamespaceURI);
                    if (!value.Equals(String.Empty))
                    {
                        sender.InnerXml = value;
            ...
            // Time
            XPathNavigator navTime = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Time' and @category='SendChatMessage']");
            if (navTime != null)
            {
                XmlElement time = document.CreateElement("Time");
                time.IsEmpty = false;
                ...
                {
                    time.InnerXml = value;
                    sendChatMessage.AppendChild(time);
                }
            }

            // Message
            XPathNavigator navMessage = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Message' and @category='SendChatMessage']");
            if (navMessage != null)
            {
                XmlElement message = document.CreateElement("Message");
                message.IsEmpty = false;
                String value = navMessage.GetAttribute("value", navMessage.NamespaceURI);
                if (!value.Equals(String.Empty))
                {
                    message.InnerXml = value;
                    sendChatMessage.AppendChild(message);
                }
            }
        }

        private void createSendVoiceMessage(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            // Create SendVoiceMessage
            XmlElement sendVoiceMessage = document.CreateElement("SendVoiceMessage");
            sendVoiceMessage.IsEmpty = false;

            parentElement.AppendChild(sendVoiceMessage);

            // Channel
            String sendVoiceMessageEventId = navLocal.GetAttribute("ID", navLocal.NamespaceURI);
            XPathNavigator navSendVoiceMessage = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='SendVoiceMessageChannel']/Component[@ID='{0}' and @Type='SendVoiceMessageEvent']", sendVoiceMessageEventId));
            if (navSendVoiceMessage != null)
            {
                XPathNavigator navOpenVoiceChannel = navSendVoiceMessage.SelectSingleNode("Component[@Type='OpenVoiceChannelEvent']");
                if (navOpenVoiceChannel != null)
                {
                    XPathNavigator navOpenVoiceChannelName = navOpenVoiceChannel.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Name' and @category='OpenVoiceChannel']");
                    if (navOpenVoiceChannelName != null)
                    {
                        ...
                        if (!value.Equals(String.Empty))
                        {
                            channel.InnerXml = value;
                            sendVoiceMessage.AppendChild(channel);
                        }
                    }
                }
            }
            ...
            // FilePath
            XPathNavigator navFileName = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='FilePath' and @category='SendVoiceMessage']");
            if (navFileName != null)
            {
                XmlElement fileName = document.CreateElement("FilePath");
                fileName.IsEmpty = false;
                String value = navFileName.GetAttribute("value", navFileName.NamespaceURI);
                if (!value.Equals(String.Empty))
                {
                    fileName.InnerXml = value;
                    sendVoiceMessage.AppendChild(fileName);
                }
            }

            // Time
            XPathNavigator navTime = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Time' and @category='SendVoiceMessage']");
            if (navTime != null)
            {
                XmlElement time = document.CreateElement("Time");
                time.IsEmpty = false;
                ...
                {
                    time.InnerXml = value;
                    sendVoiceMessage.AppendChild(time);
                }
            }
        ...
        private void createSendVoiceMessageToUser(XPathNavigator navLocal, XmlDocument document, XmlElement parentElement, XPathNavigator navGlobal)
        {
            // Create SendVoiceMessage
            XmlElement sendVoiceMessage = document.CreateElement("SendVoiceMessageToUser");
            sendVoiceMessage.IsEmpty = false;
            ...
            // Channel
            String sendVoiceMessageEventId = navLocal.GetAttribute("ID", navLocal.NamespaceURI);
            XPathNavigator navSendVoiceMessage = navGlobal.SelectSingleNode(String.Format("/LinkTypes/Link[@id='{0}' and @type='SendVoiceMessageToUserChannel']/Component[@ID='{0}' and @Type='SendVoiceMessageToUserEvent']", sendVoiceMessageEventId));
            if (navSendVoiceMessage != null)
            {
                XPathNavigator navDecisionMakers = navSendVoiceMessage.SelectSingleNode("Component[@Type='DecisionMaker']");
                if (navDecisionMakers != null)
                {
                    String navDecisionMakerID = navDecisionMakers.GetAttribute("Name", navDecisionMakers.NamespaceURI);
                    XmlElement dmID = document.CreateElement("DecisionMakerID");
                    dmID.IsEmpty = false;
                    if (!navDecisionMakerID.Equals(String.Empty))
                    {
                        dmID.InnerXml = navDecisionMakerID;
                        sendVoiceMessage.AppendChild(dmID);
                    }
                    if (navDecisionMakerID != null)
                    {
                       /* XmlElement dmID = document.CreateElement("DecisionMakerID");
                        dmID.IsEmpty = false;
                        String value = navDecisionMakerID.GetAttribute("identifier", navDecisionMakerID.NamespaceURI);
                        if (!value.Equals(String.Empty))
                        {
                            dmID.InnerXml = value;
                            sendVoiceMessage.AppendChild(dmID);
            ...
            // FilePath
            XPathNavigator navFileName = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='FilePath' and @category='SendVoiceMessageToUser']");
            if (navFileName != null)
            {
                XmlElement fileName = document.CreateElement("FilePath");
                fileName.IsEmpty = false;
            ...
            // Time
            XPathNavigator navTime = navLocal.SelectSingleNode("ComponentParameters/Parameter/Parameter[@displayedName='Time' and @category='SendVoiceMessageToUser']");
            if (navTime != null)
            {
                XmlElement time = document.CreateElement("Time");
                time.IsEmpty = false;
        ...
        private Boolean validate(IXPathNavigable iNav)
        {
            ...
            XmlReader dddSchema = settings.controller.GetXSD("DDDSchema_4_2.xsd");

            XmlReaderSettings readerSettings = new XmlReaderSettings();
            readerSettings.ValidationType = ValidationType.Schema;
            readerSettings.Schemas.Add(null, dddSchema);
    ...
    public class DDD_4_2_ExporterSettings : ControllerToXmlSettings
    {
        public DDD_4_2_ExporterSettings(Controller controller,
                                        RootController rootController,
                                        Boolean treeDataFormat) : base(controller, rootController, treeDataFormat)
        {

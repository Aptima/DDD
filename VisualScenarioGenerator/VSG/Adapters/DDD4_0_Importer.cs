...
{
    class DDD4_0_Importer : ValidationHelper, IImportAdapter
    {
        ...
        public DDD4_0_Importer(IController validatingController) : base(validatingController)
        {
        ...
        private void createEmitter(XPathNavigator navLocal, XmlDocument document, XPathNavigator navGlobal)
        {
            ...
            // Attribute could be invalid!
            createParameter(VSGConfiguration, document, cName, "Component", "Emitter.Attribute", (navLocal.SelectSingleNode("Attribute") == null) ? String.Empty : navLocal.SelectSingleNode("Attribute").Value, String.Empty);
        ...
        private void createSensor(XPathNavigator navLocal, XmlDocument document, XPathNavigator navGlobal)
        {
            ...
            // The next node should be a Attribute or Extent - position is 2 because Name is position 1 and we want the next child node.
        ...
        private void createCreateEvent(XPathNavigator navLocal, XmlDocument document, XPathNavigator navGlobal)
        {
            ...
            Int32 adoptPlatNum = 0;
            XPathNodeIterator itAdoptPlatforms = navLocal.Select("Adopt_Platform");
            while (itAdoptPlatforms.MoveNext())
            {
                itAdoptPlatforms.Current.CreateAttribute(String.Empty, "name", String.Empty, cName + "AdoptPlatform" + adoptPlatNum++ + AME.Tools.ImportTool.Delimitter + "AdoptPlatform");
                createAdoptPlatform(itAdoptPlatforms.Current, document, navGlobal);
            }
        ...
        private void createAdoptPlatform(XPathNavigator navLocal, XmlDocument document, XPathNavigator navGlobal)
        {
            String eventName = navLocal.SelectSingleNode("parent::node()").GetAttribute("name", navLocal.NamespaceURI);

            // Set up component values
            String cType = "AdoptPlatform";
            String cName = navLocal.GetAttribute("name", navLocal.NamespaceURI);
            String cDescription = String.Empty;
            // Create component
            createComponent(VSGConfiguration, document, cType, cName, cDescription);
            ...
            // AdoptPlatformUnit
            String child = (navLocal.SelectSingleNode("Child") == null) ? String.Empty : navLocal.SelectSingleNode("Child").Value;
            child = child.Trim();
            if (navGlobal.SelectSingleNode(String.Format("/Scenario/Create_Event[ID='{0}']", child)) != null)
            {
                String name = navGlobal.SelectSingleNode(String.Format("/Scenario/Create_Event[ID='{0}']", child)).GetAttribute("name", navGlobal.NamespaceURI);
                createLink(VSGConfiguration, document, cName, cName, name, "AdoptPlatformUnit", String.Empty);
            }
            ...
            createParameter(VSGConfiguration, document, cName, "Component", "AdoptPlatform.InitialState", (navLocal.SelectSingleNode("InitialState") == null) ? String.Empty : navLocal.SelectSingleNode("InitialState").Value, String.Empty);

            // Need to split this up
            String direction = (navLocal.SelectSingleNode("Location") == null) ? String.Empty : navLocal.SelectSingleNode("Location").Value;
            if (!direction.Equals(String.Empty))
            {
                String[] xyz = direction.Split(' ');
                createParameter(VSGConfiguration, document, cName, "Component", "AdoptPlatformLocation.X", xyz[0], String.Empty);
                createParameter(VSGConfiguration, document, cName, "Component", "AdoptPlatformLocation.Y", xyz[1], String.Empty);
                if (xyz.Length > 2)
                    createParameter(VSGConfiguration, document, cName, "Component", "AdoptPlatformLocation.Z", xyz[2], String.Empty);
            }

            if (navLocal.SelectSingleNode("InitialParameters") != null)
            {
                XPathNodeIterator itChildNodes = navLocal.Select("InitialParameters/child::Parameter");
                while (itChildNodes.MoveNext())
                {
        ...
        private void createLaunchEvent(XPathNavigator navLocal, XmlDocument document, XPathNavigator navGlobal)
        {
            ...
            // LaunchEventSubplatform - Kind only... skipping Child in this release
            String subplatform = (navLocal.SelectSingleNode("Kind") == null) ? String.Empty : navLocal.SelectSingleNode("Kind").Value;
            subplatform = subplatform.Trim();
            if (navGlobal.SelectSingleNode(String.Format("/Scenario/Create_Event[ID='{0}']/Subplatform[Kind='{1}']", id, subplatform)) != null)
            {
                subplatform = navGlobal.SelectSingleNode(String.Format("/Scenario/Create_Event[ID='{0}']/Subplatform[Kind='{1}']", id, subplatform)).GetAttribute("name", navGlobal.NamespaceURI);
            ...
            while (itEvents.MoveNext())
            {
                ...
                switch (itEvents.Current.Name)
                {
                    ...
                    case "Completion_Event":
                        ...
                        break;
                    case "Create_Event":
                        itEvents.Current.CreateAttribute(String.Empty, "name", navGlobal.NamespaceURI, itEvents.Current.Name + eventNum++ + AME.Tools.ImportTool.Delimitter + itEvents.Current.SelectSingleNode("ID").Value);
                        break;
                    case "FlushEvents":
        ...
        private Boolean validate(String uri, out IXPathNavigable iNav)
        {
            ...
            XmlReader schemaDatabase = validatingController.GetXSD("DDDSchema_4_0.xsd");

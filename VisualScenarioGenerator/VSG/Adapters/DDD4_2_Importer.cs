...
{
    class DDD4_2_Importer : ValidationHelper, IImportAdapter
    {
...
//        private const String ProjectConfiguration = "Project";
        ...
        public DDD4_2_Importer(IController validatingController) : base(validatingController)
        {
        ...
        private void createActiveRegion(XPathNavigator navLocal, XmlDocument document, XPathNavigator navGlobal)
        {
            ...
            String refPoint = (navLocal.SelectSingleNode("ReferencePoint") == null) ? String.Empty : navLocal.SelectSingleNode("ReferencePoint").Value;
            if (!refPoint.Equals(String.Empty))
            {
                //String[] xyz = refPoint.Split(' ');
                //createParameter(VSGConfiguration, document, cName, "Component", "ReferencePoint.X", xyz[0], String.Empty);
                //createParameter(VSGConfiguration, document, cName, "Component", "ReferencePoint.Y", xyz[1], String.Empty);
               // if (xyz.Length > 2)
                 //   createParameter(VSGConfiguration, document, cName, "Component", "ReferencePoint.Z", xyz[2], String.Empty);
                //else
                 //   createParameter(VSGConfiguration, document, cName, "Component", "ReferencePoint.Z", "0", String.Empty);
            }
            else
            {//defaults for backwards compatibility
                refPoint = "0 0";
                
                //createParameter(VSGConfiguration, document, cName, "Component", "ReferencePoint.X", "0", String.Empty);
                //createParameter(VSGConfiguration, document, cName, "Component", "ReferencePoint.Y", "0", String.Empty);
                //createParameter(VSGConfiguration, document, cName, "Component", "ReferencePoint.Z", "0", String.Empty);
            }
            createParameter(VSGConfiguration, document, cName, "Component", "Location.ReferencePoint", refPoint, String.Empty);
            String absoluteVertexList;
            PolygonHelper.DeriveAbsoluteVerticesFromRelative(refPoint, String.Join(", ", verticies.ToArray()), out absoluteVertexList);
            createParameter(VSGConfiguration, document, cName, "Component", "Location.Polygon Points", absoluteVertexList, String.Empty);
            

            createParameter(VSGConfiguration, document, cName, "Component", "Active Region.Is this a Dynamic Region", (navLocal.SelectSingleNode("IsDynamicRegion") == null) ? String.Empty : navLocal.SelectSingleNode("IsDynamicRegion").Value, String.Empty);
                ...
                while (itChildNodes.MoveNext())
                {
                    switch (itChildNodes.Current.Value)
                    {
                        ...
                        case "ObjectName":
                            ...
                            break;
                        case "LinkedRegion":
                            createLink(VSGConfiguration, 
                                document,
                                cName,
                                cName, 
                                itChildNodes.Current.SelectSingleNode("following-sibling::node()").Value,                                 
        ...
        private Boolean validate(String uri, out IXPathNavigable iNav)
        {
            ...
            XmlReader schemaDatabase = validatingController.GetXSD("DDDSchema_4_2.xsd");

...
{
    public partial class EvtPnl_Launch : UserControl, AME.Views.View_Components.IViewComponent
    {
        ...
        public void UpdateViewComponent()
        {
            ...
            if (DisplayID == subplatformLinkBox.ConnectFromId)
            {
                if (createID != subplatformLinkBox.DisplayRootId)
                {
                    subplatformLinkBox.DeleteAllLinks();
                }
            }

            subplatformLinkBox.DisplayRootId = createID;
            subplatformLinkBox.DisplayComponentType = "CreateEvent";
            subplatformLinkBox.DisplayLinkType = "CreateEventSubplatform";
            subplatformLinkBox.ConnectRootId = DisplayID;
            subplatformLinkBox.ConnectFromId = DisplayID;
            subplatformLinkBox.ConnectLinkType = "LaunchEventSubplatform";
            subplatformLinkBox.OneToMany = false;
            subplatformLinkBox.UpdateViewComponent();

            ids = myController.GetChildIDs(DisplayID, "CreateEvent", "LaunchEventSubplatform");
            ...
            else
            {
                ...
                if (ids.Count >= 1)
                {
                    ...
                    initialStateComboBox.ParameterCategory = "LaunchEvent";

...
{
    public partial class CustomParameterTextBox : TextBox, IViewComponent
    {
        ...
        private IFormatProvider displayFormatter;
        private NumberStyles numberStyle = NumberStyles.None;
        private Type formatType; // discovered at runtime
        private String formatString = "{0:N}"; // number - ex. converts 1000000 to 1,000,000.00
        private Type stringType = typeof(String);
        private Type numberStylesType = typeof(NumberStyles);

        public IFormatProvider DisplayFormatter
        {
            set
            {
        ...
        public CustomParameterTextBox(IContainer container) : this()
        {
            container.Add(this);
        }
        ...
        public void SetParameterValue(bool sendUpdateEvent)
        {
            if (controller != null)
            {
                try    
                {
                    ...
                    //if(sendUpdateEvent)
                        controller.UpdateParameters(componentId, parameterCategory + "." + parameterName, value, parameterType, sendUpdateEvent);
                }
                catch (Exception ex)
                {

...
//        private int kiOutputDeviceId = 6;
        ...
        {
            if (m_curSelectedChannel != String.Empty)
            {
                if (_voiceChannels.ContainsKey(m_curSelectedChannel))
                {
                    if (_voiceChannels[m_curSelectedChannel].IsJoined())
                    {
                        joinButton.Enabled = true;
                        joinButton.Text = "Leave";

                        talkCheckbox.Checked = _voiceChannels[m_curSelectedChannel].Talk;
                        talkCheckbox.Enabled = true;
                    }
                    else
                    {
                        joinButton.Enabled = true;
        ...
        {
            String channelName = m_voipclient.GetChannelName((int)e.Node.Tag);
            String userName = m_voipclient.GetUserNickName((int)e.Node.Tag);
            if (channelName != null && _voiceChannels.ContainsKey(channelName))
            {
                ...
                m_curSelectedChannel = joinchannelTextBox.Text;
                updateButtonStates();
            }
            else
            {
                m_curSelectedChannel = String.Empty;
                joinchannelTextBox.Text = String.Empty;
                updateButtonStates();
            }

            //if (e.Node.ImageIndex == (int)ImageIndex.CHANNEL)
            //{
            //    joinchannelTextBox.Text = m_voipclient.GetChannelName((int)e.Node.Tag);
            //    m_curSelectedChannel = joinchannelTextBox.Text;
            //    updateButtonStates();
        ...
        private void sendDDDPushToTalkEvent()
        {
            ...
            m_eventCommunicator.sendPushToTalkVoiceChannelEvent(m_curSelectedChannel);
        }
        ...
        private void sendDDDStoppedTalkingEvent()
        {
            ...
            m_eventCommunicator.sendStoppedTalkingVoiceChannelEvent(m_curSelectedChannel);
        }
        ...
        private void talkCheckbox_CheckedChanged(object sender, EventArgs e)
        {
            if (m_curSelectedChannel != String.Empty && _voiceChannels.ContainsKey(m_curSelectedChannel))
            {
                _voiceChannels[m_curSelectedChannel].Talk = talkCheckbox.Checked;
            }
        }
